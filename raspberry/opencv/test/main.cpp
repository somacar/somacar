...
public:
    ...
    OCR(bool d, int reg, int g, int rec);
...
private:
    ...
    int GROUPING;
    int RECOG;
    vector<Ptr<OCRTesseract>> tess;
    vector<Ptr<OCRHMMDecoder>> hmm;
    vector<Mat> channels;
...
OCR::OCR(bool downsize, int region, int group, int recog) {
...
void OCR::initTess(int num) {
    for (int o = 0; o < num; o++) {
        this->tess.push_back(OCRTesseract::create());
    }
}

void OCR::initHMM(int num) {
    Mat transition_p;
    ...
    for (int o = 0; o < num; o++) {
        this->hmm.push_back(OCRHMMDecoder::create(loadOCRHMMClassifierNM("OCRHMM_knn_model_data.xml.gz"),
                                                  voc, transition_p, emission_p));
    }
}

void OCR::setImage(Mat img) {
    if (this->downsize)
...
void OCR::detect() {
    cvtColor(this->img, this->grey, CV_BGR2GRAY);
    this->channels.clear();
    this->channels.push_back(grey);
    this->channels.push_back(255 - grey);

    vector<ERStat> v;
    this->regions.push_back(v);
    this->regions.push_back(v);
    this->regions[0].clear();
    this->regions[1].clear();
}

void OCR::region() {
    switch (this->REGION) {
        case 0: {
            parallel_for_(Range(0, (int) this->channels.size()), Parallel_extractCSER(this->channels, this->regions, this->erf1, this->erf2));
            break;
        }
        case 1: {
...
void OCR::group() {
    switch (this->GROUPING) {
        case 0: {
            erGrouping(this->img, this->channels, this->regions, this->nm_region_groups, this->nm_boxes, ERGROUPING_ORIENTATION_HORIZ);
            break;
        }
        case 1: {
            erGrouping(this->img, this->channels, this->regions, this->nm_region_groups, this->nm_boxes, ERGROUPING_ORIENTATION_ANY, "./trained_classifier_erGrouping.xml", 0.5);
            break;
        }
    }
}
...
void OCR::recog() {
    ...
    for (int i = 0; i < (int) this->nm_boxes.size(); i++) {
        rectangle(this->out, this->nm_boxes[i].tl(), this->nm_boxes[i].br(), Scalar(255, 255, 0), 3);


        Mat group_img = Mat::zeros(this->img.rows + 2, this->img.cols + 2, CV_8UC1);
        er_draw(this->channels, this->regions, this->nm_region_groups[i], group_img);
        group_img(this->nm_boxes[i]).copyTo(group_img);
...
int main(int argc, char *argv[]) {
    ...
    char *recognitions_str = const_cast<char *>("NM_chain_features + KNN");
...
//        vector<Mat> detections;
        ...
        putText(out_img, recognitions_str, Point(out_img.rows - (150 / scale), out_img.rows - (10 / scale)), FONT_HERSHEY_DUPLEX, scale_font, Scalar(255, 0, 0));
