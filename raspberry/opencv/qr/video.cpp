...
int main(int argc, char **argv) {
    ...
    while (true) {
        ...
        mark = 0;   // Reset all detected marker count
        ...
        if (mark >= 2) {
            // We have found the 3 markers for the QR code;
            // Now we need to determine which of them are 'top', 'right' and 'bottom' markers
            ...
            if (top < contours.size() && right < contours.size() && bottom < contours.size() && contourArea(contours[top]) > 10 && contourArea(contours[right]) > 10 &&
