...
//  Serial.println(F(""));
  ...
  int overLevel = checkOverturn(accel_t_gyro.value.z_accel);
  

  
  if (overLevel >= 3) {
    overTurnCount = overTurnCount + 1;
  } else {
  ...
  if (overTurnCount > 3) {
    Serial.println("-------------------------------------------");
    Serial.println("[Overturn] Overturn Detected - (exceed 4)");
    Serial.println("-------------------------------------------");
  }
 
  Serial.print(">>>>>>>>>>>>>Check_Overturn_Level : ");
  Serial.println(overLevel);
  Serial.print(">>>>>>>>>>>>>Check_Overturn_Count : ");
  Serial.println(overTurnCount);
  delay(200);
}

int checkOverturn(int z_accel) {
  //overTurnLevel = 0 (init)
  if (z_accel >= 15000) {
    return 0;
  } else if (z_accel >= 14000)  {
    return 1;
  } else if (z_accel >= 12000) {
    return 2;
  } else if (z_accel >= 10000) {
    return 3;
  } else if (z_accel >= 8000) {
    return 4;
  } else if (z_accel >= 6000) {
    return 5;
  } else if (z_accel >= 4000) {
    return 6;
  } else if (z_accel >= 2000) {
    return 7;
  } else if (z_accel >= 0) {
    return 8; 
  } else if (z_accel >= -2000) {
